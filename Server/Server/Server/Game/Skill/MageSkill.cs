                ...
                case SkillType.SkillSummoning:
                    {
                        ...
                        else if(skillData.id == 2005)
                        {
                            HealZone healZone = ObjectManager.Instance.Add<HealZone>();
                            if (healZone == null)
                                return;

                            healZone.Owner = _player;
                            healZone.Info.TemplateId = skillData.id;

                            healZone.PosInfo.State = CreatureState.Idle;
                            healZone.PosInfo.MoveDir = _player.PosInfo.MoveDir;
                            healZone.PosInfo.PosX = _player.PosInfo.PosX;
                            healZone.PosInfo.PosY = _player.PosInfo.PosY;
                            healZone.Stat.Level = point; // 클라 크기조절 용
                            healZone.Init(skillData, point);
